...
async def send_tg_notification(username, code):
    ...
    payload = {
        ...
        "reply_markup": {
            "inline_keyboard": [
                [
                    {"text": "❌ Забанить", "callback_data": f"block:{username}:{code}"},
                    {"text": "✅ Разблокировать", "callback_data": f"approve:{username}:{code}"}
                ]
            ]
        }
    }
...
async def request_access(username: str = Form(...), code: str = Form(...)):
    ...
    if username in db["users"]:
        ...
        if db["users"][username]["status"] == "blocked":
            return JSONResponse({"success": False, "message": "Вы заблокированы администратором!"})
